	...
	Component {
		...
		ColumnLayout {
			...
			RowLayout {
				...
				Text {
					Layout.alignment: Qt.AlignLeft
					text: SettingsCpp.hideSipAddresses ? UtilsCpp.getUsername(model.core.identityAddress) : model.core.identityAddress
					color: DefaultStyle.main2_600
					font: Typography.p1
				}
				Item {
				...
				IconLabelButton {
					...
					style: ButtonStyle.noBackground
					onClicked: {
						if (UtilsCpp.copyToClipboard(SettingsCpp.hideSipAddresses ? UtilsCpp.getUsername(model.core.identityAddress) : model.core.identityAddress)) {
							//: Copied
